...
public class AutoAuthority {
	...
	public void defaultAuthor(EmployeeBean emp) {
		...
		if(personalHead || CEO) {//인사부장,사장 모든권한 부
			List<AuthInfoBean> allAuth = authInfoDao.getAllAuth();
			...
			String alreadyAuth1 = empAuthDao.getAuthByEmpNo(emp.getEmp_no(),writeNotice);
			if(alreadyAuth1 == null) {
				int cnt1 = empAuthDao.insertEmployeeAuth(emp.getEmp_no(), writeNotice);
				
			}
			String alreadyAuth2 = empAuthDao.getAuthByEmpNo(emp.getEmp_no(), adminAuth);
			if(alreadyAuth2 == null) {
				empAuthDao.insertEmployeeAuth(emp.getEmp_no(), adminAuth);
			}
