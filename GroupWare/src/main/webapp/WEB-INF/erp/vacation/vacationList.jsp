...
<div class="container mt-5">
	...
	<div class="row mb-3">
		<div class="col-md-4">
			<input type="text" class="form-control" placeholder="신청자 검색..."
				id="searchInput">
		</div>
		<div class="col-md-4">
			<select class="form-control" id="statusFilter" name="whatColumn">
		...
		<table
			...
			<tbody>
				...
				<c:if test="${empty vacationList}">
					<tr>
						<td colspan="9" class="text-center text-muted">현재 신청된 휴가가
							없습니다.</td>
					</tr>
				</c:if>
				...
				<c:if test="${not empty vacationList}">
					<c:forEach var="vacation" items="${vacationList}">
						<tr>
							...
							<td><span class="text-primary"><fmt:formatDate
										value="${vacation.start_dt}" pattern="yyyy-MM-dd" /></span> ~ <span
								class="text-danger"><fmt:formatDate
										value="${vacation.end_dt}" pattern="yyyy-MM-dd" /></span></td>
							...
							<td>
								<button class="btn btn-success btn-sm"
									onclick="confirmApproval(${vacation.vacation_no})">✔
									승인</button>
							</td>
							<td>
								<button class="btn btn-danger btn-sm"
									onclick="confirmRejection(${vacation.vacation_no})">❌
									반려</button>
							</td>
						</tr>
					</c:forEach>
