    ...
    <c:if test="${not empty vacationList}">
        ...
        <c:forEach var="vacation" items="${vacationList}">
            <div style="display: grid; grid-template-columns: 
                ...
                <div>${vacation.appr_emp_nm}</div>
  				<c:if test="${vacation.vacation_status == '대기'}">
                <c:if test="${sessionScope.position_cd <= 2}">
                    <div><a href="javascript:void(0)" onclick="vacationConfirm('${vacation.vacation_no}','appr')" style="color: green; font-weight: bold;">✔</a></div>
                    <div><a href="javascript:void(0)" onclick="vacationConfirm('${vacation.vacation_no}','reject')" style="color: red; font-weight: bold;">❌</a></div>
                </c:if>
                </c:if>
            </div>
        </c:forEach>
